...
const content = document.querySelectorAll('.france-country');
const tab = document.querySelectorAll('.france-link');
const slider = document.querySelector('.france-countries-slider');
...
function hideTabContent() {
  ...
  tab.forEach(item => {
    item.classList.remove('france-active');
  });
}

function showTabContent(i = 0) {
  content[i].style.display = 'grid';
  tab[i].classList.add('france-active');
}
